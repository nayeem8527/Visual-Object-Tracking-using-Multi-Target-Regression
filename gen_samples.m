function [ bb_samples ] = gen_samples(type, bb, n, trans_f, scale_f,img)
% GEN_SAMPLES
...
switch (type)
    case 'gaussian'
        ...
        samples(:,3:4) = samples(:,3:4) .* repmat(1.05.^(scale_f*max(-1,min(1,0.5*randn(n,1)))),1,2);
    case 'uniform'
        samples(:,1:2) = samples(:,1:2) + trans_f * round(mean(bb(3:4))) * (rand(n,2)*2-1);
        samples(:,3:4) = samples(:,3:4) .* repmat(1.05.^(scale_f*(rand(n,1)*2-1)),1,2);
    case 'uniform_aspect'
        samples(:,1:2) = samples(:,1:2) + trans_f * repmat(bb(3:4),n,1) .* (rand(n,2)*2-1);
        samples(:,3:4) = samples(:,3:4) .* 1.05.^(rand(n,2)*4-2);
        samples(:,3:4) = samples(:,3:4) .* repmat(1.05.^(scale_f*rand(n,1)),1,2);
    case 'whole'
        ...
        windows = [dx(:) dy(:) bb(3)*1.05.^ds(:) bb(4)*1.05.^ds(:)];
