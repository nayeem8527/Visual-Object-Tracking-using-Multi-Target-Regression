...
for n = 4:length(seq)
    ...
    dataPath = strcat(dataPath,'img/');
    img = imread([dataPath files(1).name]);
    if size(img,3) == 3
        img = rgb2gray(img);
    end
...
%     p1=0.3;
    for f = 2 : frameNum
%         try
            frame = imread([dataPath files(f).name]);
...
%             if f==2                
            [bb_next, feats, pred,ind,gt] = do_tracking(frame, dict, bb_prev, featpars, trackpars, dictpars.sparsity, opt,p1,1,gt);
%             else
...
%             end
            ...
            if ~isempty(feats)
                cnt = cnt + 1;
                samples.feaArr = [samples.feaArr feats.feaArr];
                samples.label = [samples.label feats.label'];
                samples.boxes = [samples.boxes feats.boxes'];
    %             samples_ldp.feaArr = [samples_ldp.feaArr feats_ldp.feaArr];
    %             samples_ldp.label = [samples_ldp.label feats_ldp.label'];
%                 disp('get template');
%                 tic
                [dict.T, dict.TW] = get_template(frame, bb_next, dict.T, dict.TW, max(pred), trackpars,featpars.bbox);
